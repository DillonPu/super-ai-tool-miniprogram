// pages/mini_lookFruitsAndVeg/miniLookFruitsAndVeg.js
...
Page({
  ...
  binBtnLook: function (e) {
    ...
    superAiApi.getBaidubceToken({"code":"1"}).then(token => {
      this._ingredient(token.data);
    });
  },
  // 百度果蔬识别
  async _ingredient(token) {
    let imageUrl = this.data.imageTempUrl;
    // 图片转base64
    let base64 = wx.getFileSystemManager().readFileSync(imageUrl, "base64");
    let result = await baidubceApi._ingredient(token, base64);
